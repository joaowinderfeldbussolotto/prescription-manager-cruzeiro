...
def test_as_date_from_datetime():
    assert as_date(datetime(2025, 1, 10, 12, 0, tzinfo=timezone.utc)) == date(2025, 1, 10)


def test_as_date_from_date():
    assert as_date(date(2025, 1, 10)) == date(2025, 1, 10)
...
def test_comparacao_apos_normalizacao_nao_quebra():
    ...
    validade = as_date(date(2026, 1, 10))
    emissao = as_date(datetime(2025, 1, 10, tzinfo=timezone.utc))
    assert validade > emissao  # não levanta TypeError


def test_default_validade_quando_limpa():
    emissao = as_date(datetime(2025, 3, 1, tzinfo=timezone.utc))
